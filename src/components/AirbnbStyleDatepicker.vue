...
export default {
	...
	watch: {
		selectedDate1(newValue, oldValue) {
			let newDate =
				!newValue || newValue === ''
					? ''
					: format(parseISO(newValue), this.dateFormat, { awareOfUnicodeTokens: true })
	...
	methods: {
		...
		isMonthDisabled(year, monthIndex) {
			...
			if (this.hasMinDate && isBefore(monthDate, startOfMonth(parseISO(this.minDate)))) {
				return true
			}
		...
		getMonth(date) {
			const firstDateOfMonth = format(parseISO(date), 'YYYY-MM-01', { awareOfUnicodeTokens: true })
			const year = format(parseISO(date), 'YYYY', { awareOfUnicodeTokens: true })
			const monthNumber = parseInt(format(parseISO(date), 'M', { awareOfUnicodeTokens: true }))
		...
		getWeeks(date) {
			...
			format(parseISO(date), 'YYYY', { awareOfUnicodeTokens: true })
			const year = format(parseISO(date), 'YYYY', { awareOfUnicodeTokens: true })
			const month = format(parseISO(date), 'MM', { awareOfUnicodeTokens: true })
			let firstDayInWeek = parseInt(format(parseISO(date), this.sundayFirst ? 'd' : 'E'), {
		...
		setFocusedDate(date) {
			const formattedDate = format(isDate(date) ? date : parseISO(date), this.dateFormat, {
				awareOfUnicodeTokens: true,
			})
		...
		isToday(date) {
			return format(new Date(), this.dateFormat, { awareOfUnicodeTokens: true }) === date
		},
		...
		isInRange(date) {
			...
			return (
				...
				(isAfter(parseISO(date), parseISO(this.selectedDate1)) &&
					isBefore(parseISO(date), parseISO(this.hoverDate)) &&
		...
		isHoveredInRange(date) {
			...
			return (
				(isAfter(parseISO(date), parseISO(this.selectedDate1)) &&
					isBefore(parseISO(date), parseISO(this.hoverDate))) ||
				(isAfter(parseISO(date), parseISO(this.hoverDate)) &&
					isBefore(parseISO(date), parseISO(this.selectedDate1)))
			)
		},
		...
		isAfterEndDate(date) {
			...
			return isAfter(parseISO(date), this.endDate)
		},
		isDateVisible(date) {
			if (!date) {
				return false
			...
			return isAfter(parseISO(date), parseISO(start)) && isBefore(parseISO(date), parseISO(end))
		},
		isDateDisabled(date) {
		...
		subtractMonths(date) {
			return format(subMonths(isDate(date) ? date : parseISO(date), 1), this.dateFormat, {
				awareOfUnicodeTokens: true,
			})
		},
		addMonths(date) {
			return format(addMonths(parseISO(date), 1), this.dateFormat, { awareOfUnicodeTokens: true })
		},
		toggleDatepicker() {
